...
# 엑셀 파일 경로 설정
file_path = "/content/drive/MyDrive/sample_200.xlsx"
...
# 라벨별 198개 균등 추출
balanced_df = df.groupby('label').sample(n=198, random_state=42).reset_index(drop=True)
...
# 평가 지표 함수 수정 (accuracy → eval_accuracy로 반환)
...
# 학습 설정 (eval_accuracy 기준으로 best 모델 저장)
training_args = TrainingArguments(
    ...
    metric_for_best_model="eval_accuracy",  # <-- accuracy 기준 저장
    greater_is_better=True,                 # <-- 높은 accuracy가 좋다고 설정
    report_to="none"
)
